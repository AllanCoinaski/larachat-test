...
export default {
    computed: {
        ...mapState({
            ...
            messages: (state) => state.chat.messages,//seta as mensagens no chat
            me: (state) => state.me.me,
        }),

        disabledButton () {//desabilitar o botão se a qtd de caracteres for menor q um
  ...
  //méotodo que faz os scroll das mensagens
        scrollMessages () {
            setTimeout(() => {
        ...
        sendMessage () {//submeter o formulario
            ...
            this.sendNewMessage(this.message)
                    .then(response => {
                        this.message = ''//envia e zera a variavel message
                    })
    ...
    watch: { //escuta o computed messages para chamar o scroll automatico
        messages () {
            this.scrollMessages()
        }
    }
